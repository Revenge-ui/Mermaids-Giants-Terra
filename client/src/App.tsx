...
import { useBattleMotion } from "./battle/useBattleMotion";
import { withRequestTimeout } from "./ui/requestTimeout";
import { canInteractWithBattle, emptySelection, isCardPlayable, isLegalTarget, isYourTurn, presentationFromEvents, targetModeForCard, type PendingAction, type PresentationState, type SelectionState } from "./uiModel";
...
export function App() {
  ...
  const [turnBanner, setTurnBanner] = useState<PresentationState["turnBanner"]>();
  const [showIntro, setShowIntro] = useState(false);
  const [showMulliganResult, setShowMulliganResult] = useState(false);
  const [mulliganSelected, setMulliganSelected] = useState<string[]>([]);
  useBattleMotion(battleRoot, presentation);
  // Preserve the server turn event until opening overlays have finished.
  useEffect(() => {
    if (!turnBanner || showIntro || showMulliganResult) return;
    const timer = window.setTimeout(() => setTurnBanner(undefined), 1050);
    return () => window.clearTimeout(timer);
  }, [turnBanner, showIntro, showMulliganResult]);
  ...
  useEffect(() => { if (!showMulliganResult) return; const timer = window.setTimeout(() => setShowMulliganResult(false), 1100); return () => window.clearTimeout(timer); }, [showMulliganResult]);
  useEffect(() => {
  ...
  useEffect(() => {
    const onGame = (nextGame: PlayerViewState, events: GameEvent[]) => {
      ...
      const nextPresentation = presentationFromEvents(events, nextGame.you.playerId, previous, presentationNonce.current);
      setPresentation(nextPresentation);
      if (previous?.gameId !== nextGame.gameId) setTurnBanner(undefined);
      if (nextPresentation.turnBanner) setTurnBanner(nextPresentation.turnBanner);
      playSounds(events, nextGame.you.playerId);
      if ((!previous || previous.gameId !== nextGame.gameId) && nextGame.status === "MULLIGAN") setShowIntro(true);
      if (previous?.status === "MULLIGAN" && nextGame.status === "PLAYING") setShowMulliganResult(true);
      if (!previous || previous.gameId !== nextGame.gameId || nextGame.mulliganConfirmed) setMulliganSelected([]);
      gameRef.current = nextGame; setGame(nextGame); setSelection(emptySelection(nextGame.gameId)); setInspectedCard(null);
    };
    const unsubscribers = [
  ...
  const canAct = Boolean(game && canInteractWithBattle(game, connectionStatus === "CONNECTED", pending, showIntro || showMulliganResult || confirmSurrender));
  const enemyTargetable = Boolean(game && canAct && (selection.attackerId || targetMode === "ENEMY"));
  const friendlyTargetable = Boolean(game && canAct && targetMode === "FRIENDLY_MINION");
  ...
  const perform = async (kind: Exclude<PendingAction, null>, action: () => ReturnType<typeof gameClient.endTurn>) => {
    if (pendingRef.current || connectionStatus !== "CONNECTED") return;
    pendingRef.current = true; setPending(kind);
    try { const result = await withRequestTimeout(action()); if (!result.ok) setError(errorMessage(result.errorCode)); }
    catch { setError("操作未能确认，正在重新获取服务器状态。"); gameClient.disconnect(); gameClient.connect(); }
    finally { pendingRef.current = false; setPending(null); }
  };
  const playFromHand = (card: CardView) => {
    if (!canAct || pendingRef.current) return;
    if (!yourTurn) return setError("现在不是你的回合。");
    if (!isCardPlayable(game, card, pending)) return setError(card.type === "MINION" && game.you.board.length >= 7 ? "战场已满，最多容纳 7 个随从。" : "法力不足，无法打出这张牌。");
  ...
  return (
    <main ref={battleRoot} className={`game-shell faction-${game.you.faction.toLowerCase()} ${yourTurn ? "your-turn" : "enemy-turn"} ${pending ? "is-pending" : ""}`} onClick={(event) => { if (event.target === event.currentTarget) cancelSelection(); }}>
      <header className="game-header"><div className="room-mark"><span className="tiny-label">牌局</span><strong>{game.roomId}</strong></div><div className="turn-status"><span>{game.status === "MULLIGAN" ? "起手准备" : `第 ${game.turn} 回合`}</span><b>{hint}</b></div><div className="header-actions"><div className={`server-dot ${connectionStatus === "CONNECTED" ? "online" : ""}`}><i />{connectionText(connectionStatus)}</div><button className="surrender" onClick={(event) => { event.stopPropagation(); setConfirmSurrender(true); }} disabled={game.status !== "PLAYING" || pending !== null}>认输</button></div></header>
      ...
      <section className="battle-table" onClick={cancelSelection}>
        ...
        <div className="enemy-zone"><div className="opponent-hand" aria-label={`对手有 ${game.opponent.handCount} 张手牌`}>{Array.from({ length: game.opponent.handCount }, (_, index) => <i key={index} style={{ "--back-i": index, "--back-count": game.opponent.handCount } as React.CSSProperties} />)}<span>对方手牌 {game.opponent.handCount}</span></div><div className="hero-row enemy-row"><Hero player={game.opponent} enemy targetable={enemyTargetable} impacted={impactedIds.has(game.opponent.playerId)} floating={presentation.floatingNumbers.filter((cue) => cue.targetId === game.opponent.playerId)} onClick={enemyTargetable ? () => chooseEnemyTarget({ type: "HERO", playerId: game.opponent.playerId }) : undefined} /><div className="opponent-resources"><span>牌库 {game.opponent.deckCount}</span><Mana current={game.opponent.mana} max={game.opponent.maxMana} temporary={game.opponent.temporaryMana} compact /></div></div><Board minions={game.opponent.board} owner="OPPONENT" targetableIds={targetableEnemyIds} summonedIds={summonedIds} dying={presentation.dyingMinions} attackingId={presentation.attackingId} impactedIds={impactedIds} floating={presentation.floatingNumbers} onMinion={(minion) => chooseEnemyTarget({ type: "MINION", playerId: game.opponent.playerId, instanceId: minion.instanceId })} /></div>
        <div className="rift-line"><span>✦</span></div>
        <div className="friendly-zone"><Board minions={game.you.board} owner="YOU" canAct={canAct} selectedAttacker={selection.attackerId} targetableIds={targetableFriendlyIds} summonedIds={summonedIds} dying={presentation.dyingMinions} attackingId={presentation.attackingId} impactedIds={impactedIds} floating={presentation.floatingNumbers} onMinion={chooseFriendlyMinion} /><div className="hero-row friendly-row"><div className="local-resources"><Mana current={game.you.mana} max={game.you.maxMana} temporary={game.you.temporaryMana} /><span>魔力源泉</span></div><Hero player={game.you} impacted={impactedIds.has(game.you.playerId)} floating={presentation.floatingNumbers.filter((cue) => cue.targetId === game.you.playerId)} /><button className="end-turn" onClick={(event) => { event.stopPropagation(); void perform("END_TURN", () => gameClient.endTurn()); }} disabled={!canAct}><span>{pending === "END_TURN" ? "确认中" : yourTurn ? "结束回合" : "对手回合"}</span><i /></button></div></div>
      </section>
      <Hand game={game} selectedId={selection.cardInstanceId} inspectedCard={inspectedCard ?? selectedCard ?? null} drawnCardId={presentation.drawnCardId} pending={pending} canAct={canAct} onInspect={setInspectedCard} onPlay={playFromHand} />
      {presentation.playedCard && <div className="played-card-flash" key={presentation.nonce}><CardFace card={presentation.playedCard} /></div>}
      {!showIntro && !showMulliganResult && <TurnBanner kind={turnBanner} />}
      {((game.status === "MULLIGAN" && !showIntro) || showMulliganResult) && <MulliganOverlay game={game} resolved={showMulliganResult} selectedIds={mulliganSelected} pending={pending === "MULLIGAN"} onToggle={(id) => setMulliganSelected((current) => current.includes(id) ? current.filter((value) => value !== id) : [...current, id])} onConfirm={() => void perform("MULLIGAN", () => gameClient.confirmMulligan(mulliganSelected))} />}
      {showIntro && <div className="match-intro"><div><span><FactionSigil faction={game.you.faction} />{game.you.name}<small>{FACTION_LABEL[game.you.faction]}阵营</small></span><b>VS</b><span><FactionSigil faction={game.opponent.faction} />{game.opponent.name}<small>{FACTION_LABEL[game.opponent.faction]}阵营</small></span></div></div>}
      <GameToast message={error} kind="warning" onClose={() => setError("")} />
      {confirmSurrender && <GameModal title="结束这场对决？" onClose={() => setConfirmSurrender(false)}><p>认输后本局将以失败结束。你的收藏和卡组不会受到影响。</p><div className="ui-modal-actions"><GameButton onClick={() => setConfirmSurrender(false)}>继续对战</GameButton><GameButton variant="danger" onClick={() => { setConfirmSurrender(false); void perform("SURRENDER", () => gameClient.surrender()); }}>确认认输</GameButton></div></GameModal>}
