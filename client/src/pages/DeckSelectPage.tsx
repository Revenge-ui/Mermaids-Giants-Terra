...
import { validateDeck, type DeckSubmission } from "@riftbound/shared";
...
import { createStarterDeck, deckCardCount, deckManaCurve, loadActiveDeckId, loadDecks, saveActiveDeckId, saveDecks, toDeckSubmission, type LocalDeck } from "../deckStorage";
...
export function DeckSelectPage({ mode, collection, busy, error, onSelect, onCollection, onBack }: {
  ...
  const rows = useMemo(() => decks.map((deck) => ({ deck, validation: validateDeck(deck, CARD_CATALOG, collection) })), [collection, decks]);
  ...
  return <FeatureFrame icon={mode === "AI" ? "♜" : "⚔"} eyebrow={mode === "AI" ? "石桌试炼" : "好友对战"} title="选择出战卡组" description="只有经过本地检查并由服务器再次验证的 30 张卡组才能进入对局。" onBack={onBack}>
    <div className="deck-select-list">{rows.map(({ deck, validation }) => <article className={`${selectedId === deck.id ? "selected" : ""} ${validation.valid ? "valid" : "invalid"}`} key={deck.id} onClick={() => setSelectedId(deck.id)}>
      <i>▤</i><div><h2>{deck.name}</h2><p>{validation.valid ? "✓ 合法卡组" : `⚠ ${deckValidationMessage(validation.errors[0]!)}`}</p><small>{deckCardCount(deck)} / 30 张</small></div><div className="mini-curve"><DeckManaCurve curve={deckManaCurve(deck, CARD_CATALOG)} /></div><button className="feature-primary" disabled={!validation.valid || busy} onClick={(event) => { event.stopPropagation(); useDeck(deck); }}>{busy ? "正在连接…" : mode === "AI" ? "迎战守卫" : "使用此卡组"}</button>
    </article>)}</div>
