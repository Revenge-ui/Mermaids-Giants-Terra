...
import { addCardToDeck, createLocalDeck, loadDecks, removeCardFromDeck, saveDecks, type LocalDeck } from "../deckStorage";
...
export function CollectionPage({ collection, onBack }: { collection: CollectionCounts; onBack: () => void }) {
  ...
  const createDeck = () => {
    ...
    const next = [...decks, deck]; persist(next); setNewDeckName(""); setCreating(false); setActiveDeckId(deck.id); setNotice("卡组已创建");
  };
  const chooseCard = (card: CatalogCard) => {
    if (!activeDeckId) { setSelected(card); return; }
    ...
    <section className="collection-workbench">
      ...
      <div className="collection-body">
        <section className="collection-book">
          <div className="book-grain" aria-hidden="true" /><CollectionGrid cards={visibleCards} collection={collection} editing={Boolean(activeDeckId)} onSelect={chooseCard} onDetails={setSelected} />
        ...
        <DeckPanel decks={decks} activeDeckId={activeDeckId} creating={creating} newName={newDeckName} onCreating={setCreating} onNewName={setNewDeckName} onCreate={createDeck} onOpen={setActiveDeckId} onClose={() => setActiveDeckId(null)} onRemove={removeCard} />
      </div>
